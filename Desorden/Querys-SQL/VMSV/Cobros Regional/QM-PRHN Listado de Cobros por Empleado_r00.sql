...
SELECT T3.GroupName,
       ...
       T0.DocTotal/@HN   AS Total
  FROM      PRHN.DBO.ORCT T0
 INNER JOIN PRHN.DBO.OCRD T1 ON T0.CardCode = T1.CardCode
 INNER JOIN PRHN.DBO.OSLP T2 ON T1.SlpCode  = T2.SlpCode
 INNER JOIN PRHN.DBO.OCRG T3 ON T1.GroupCode = T3.GroupCode
