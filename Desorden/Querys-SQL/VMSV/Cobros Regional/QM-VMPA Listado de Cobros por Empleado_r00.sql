...
/* SELECT FROM VMPA.DBO.ORCT T0*/
SET @FechaIni= /* T0.DocDate*/ '[%0]'

/* SELECT FROM VMPA.DBO.ORCT T0*/
SET @FechaFin= /* T0.DocDate*/ '[%1]'

/*SELECT FROM VMPA.DBO.OCRG T1*/
...
SELECT T3.GroupName,
  ...
  FROM      VMPA.DBO.ORCT T0
 INNER JOIN VMPA.DBO.OCRD T1 ON T0.CardCode = T1.CardCode
 INNER JOIN VMPA.DBO.OSLP T2 ON T1.SlpCode  = T2.SlpCode
 INNER JOIN VMPA.DBO.OCRG T3 ON T1.GroupCode = T3.GroupCode
