...
SELECT T3.GroupName,
       ...
       T0.DocTotal/@GT   AS Total
  FROM      PRGT.DBO.ORCT T0
 INNER JOIN PRGT.DBO.OCRD T1 ON T0.CardCode = T1.CardCode
 INNER JOIN PRGT.DBO.OSLP T2 ON T1.SlpCode  = T2.SlpCode
 INNER JOIN PRGT.DBO.OCRG T3 ON T1.GroupCode = T3.GroupCode
