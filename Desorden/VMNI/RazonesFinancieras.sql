...
if (@nInDesign = 1)
	begin
		...
		set @dFechaFin		= '12/31/2012 00:00:00'
		set @Tc				= 8
	end
else
	begin
...
if (@nYear=year(@dFechaFin))
	begin
		...
		exec vmni.dbo.ObtenerSaldoCuenta @dFechaIni,@dFechaFin,@cCuenta,@nSaldo output 
		...
		exec vmni.dbo.ObtenerSaldoCuenta @dFechaIniAnual,@dFechaFinAnual,@cCuenta,@nSaldo output
		...
		if (@nActivoCirculante <>0 and @nPasivoCirculante <>0)
			begin
				set @nRazonCirculante = @nActivoCirculante / @nPasivoCirculante
			end

		-- Razón Período Promedio de Cobro
		if (@nCuentasPorCobrar<>0 and @nVentasNetasAnual<>0)
			begin
		...
		if ((@nVentasBrutas-@nCostoVentas)<>0 and @nVentasBrutas<>0)
			begin
				set @nRazonMargenUtilidadBruta = (@nVentasBrutas-@nCostoVentas)/@nVentasBrutas
			end

		-- Razón Margen Utilidad Operativa
		if (@nUtilidadNeta<>0 and (@nVentasBrutas-@nDevolSobreVentas)<>0)
			begin
		...
		if (@nUtilidadNeta<>0 and @nActivosTotales<>0)
			begin
