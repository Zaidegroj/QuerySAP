...
SELECT Grupo,
		...
		sum(Neto)/@Tc as [Venta Cliente],
		sum(Cantidad) as [Cant. Cliente],
		sum(PrecioPromedio)/@Tc as PrecioPromedio,
		sum(DevCant)/@Tc as [Dev. Cant. Cliente],
		sum(DevValor)/@Tc as [Dev. Valor Cliente],
		sum(AfilCant) as [Cant. Afiliada],
		sum(AfilValor)/@Tc as [Dev. Valor Afiliada],
        sum(AfilDevCant) as [Dev. Cant. Afiliada],
		sum(AfilDevValor)/@Tc as [Dev. Valor Afiliada],
		sum(cantidad)+sum(DevCant)+ sum(AfilCant)+sum(AfilDevCant) as [Cant. Neta],
		sum(neto)/@Tc + sum(devValor)/@Tc +sum(AfilDevValor)/@Tc as [Valor Neto],
		sum(CostoTotal)/@Tc as [Costo Total],
		sum(costopromedio)/@Tc as [Costo Promedio]
FROM #Tmp_Ven
