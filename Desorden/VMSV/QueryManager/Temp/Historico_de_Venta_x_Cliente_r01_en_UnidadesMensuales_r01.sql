...
--						select ItmsGrpNam,ItemCode ,ItemName,isnull(T0.Quantity,0),isnull(T0.Price,0) from openquery(loopback,'exec vmsv.dbo.SalesHistoryByDate_r01 @dFechaIni,@dFechaFin,@sGroupIni,@sGroupFin')
--						--select * from (exec SalesHistoryByDate_r01) @dFechaIni,@dFechaFin,@sGroupIni,@sGroupFin)
			end
		if (@iMonthFirst=2)
			begin
...
select CardCode,CardName,sum(QJanuary+QFebruary+QMarch+QApril+QMay+QJune+QJuly+QAugust+QSeptember+QOctober+QNovember+QDecember) as [Unidades Venta Anual],
