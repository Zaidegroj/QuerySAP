...
FROM         cvcr.dbo.OINV AS T0 INNER JOIN
             cvcr.dbo.NNM1 AS T1 ON T0.Series = T1.Series INNER JOIN
             cvcr.dbo.OCRD AS T2 ON T0.CardCode = T2.CardCode INNER JOIN
             cvcr.dbo.OCTG AS t4 ON T0.GroupNum = t4.GroupNum INNER JOIN
             cvcr.dbo.OCRG AS T3 ON T3.GroupCode = T2.GroupCode
...
SELECT     T0.CardCode AS Cliente, T0.CardName AS Nombre, T2.CntctPrsn AS agencia, T1.SeriesName AS Serie, T0.DocNum AS Documento, t4.ExtraMonth,t4.ExtraDays, 
           ...
           (T0.DocTotal - T0.PaidToDate) * - 1 AS Saldo,
			...
			T3.GroupName AS GRUPO
FROM         cvcr.dbo.ORIN AS T0 INNER JOIN
             cvcr.dbo.NNM1 AS T1 ON T0.Series = T1.Series INNER JOIN
             cvcr.dbo.OCRD AS T2 ON T0.CardCode = T2.CardCode INNER JOIN
             cvcr.dbo.OCTG AS t4 ON T0.GroupNum = t4.GroupNum INNER JOIN
             cvcr.dbo.OCRG AS T3 ON T3.GroupCode = T2.GroupCode
