...
if (@iInDesign)=1
	...
	begin
		/* SELECT FROM cvgt.DBO.OINV T1 */
		SET @Fecha = /* T1.DocDate */ '[%0]'
		/* SELECT FROM cvgt.DBO.OCRG T2 */
		SET @GrupoIni = /* T2.GroupName */ '[%1]'
		/* SELECT FROM cvgt.DBO.OCRG T2 */
		SET @GrupoFin = /* T2.GroupName */ '[%2]'
		/* SELECT FROM cvgt.DBO.ORTT T0 */
		SET @tc    = /* T0.Rate    */ '[%3]'
...
SELECT T3.GroupName AS Grupo,
	T0.CardCode                                 AS Cliente  ,
      ...
      (T0.DocTotal   - T0.PaidToDate)              AS Saldo    ,
						...
						case 
							...
							when T2.CntctPrsn not like '%DIRECTO%' and DATEDIFF(DD, cvsv.dbo.ObtenerFechaFinal(dateadd(dd,t4.ExtraDays,dateadd(mm,t4.ExtraMonth,T0.DocDate))),@Fecha) <= 0 then (T0.DocTotal - T0.PaidToDate) 
							when T2.CntctPrsn not like '%DIRECTO%' and T0.u_diasAgencia is not null and DATEDIFF(DD, dateadd(dd,t0.u_DiasAgencia,T0.DocDate),@Fecha) <= 0 then (T0.DocTotal - T0.PaidToDate) end as Normal,
						case 
							...
							when T2.CntctPrsn not like '%DIRECTO%' and DATEDIFF(DD,cvsv.dbo.ObtenerFechaFinal(dateadd(dd,t4.ExtraDays,dateadd(mm,t4.ExtraMonth,T0.DocDate))), @Fecha) >= 1 AND DATEDIFF(DD,cvsv.dbo.ObtenerFechaFinal(dateadd(dd,t4.ExtraDays,dateadd(mm,t4.ExtraMonth,T0.DocDate))), @Fecha) <= 30 THEN (T0.DocTotal - T0.PaidToDate) 
						...
						case 
							...
							when T2.CntctPrsn not like '%DIRECTO%' and DATEDIFF(DD, cvsv.dbo.ObtenerFechaFinal(dateadd(dd,t4.ExtraDays,dateadd(mm,t4.ExtraMonth,T0.DocDate))), @Fecha) >= 31 AND DATEDIFF(DD, cvsv.dbo.ObtenerFechafinal(dateadd(dd,t4.ExtraDays,dateadd(mm,t4.ExtraMonth,T0.DocDate))), @Fecha) <= 60 THEN (T0.DocTotal - T0.PaidToDate) 
						...
						case 
							...
							when T2.CntctPrsn not like '%DIRECTO%' and DATEDIFF(DD, cvsv.dbo.ObtenerFechaFinal(dateadd(dd,t4.ExtraDays,dateadd(mm,t4.ExtraMonth,T0.DocDate))), @Fecha) >= 61 AND DATEDIFF(DD,cvsv.dbo.ObtenerFechaFinal(dateadd(dd,t4.ExtraDays,dateadd(mm,t4.ExtraMonth,T0.DocDate))), @Fecha) <= 90 THEN (T0.DocTotal - T0.PaidToDate)
						...
						case 
							...
							when T2.CntctPrsn not like '%DIRECTO%' and DATEDIFF(DD, cvsv.dbo.obtenerFechafinal(dateadd(dd,t4.ExtraDays,dateadd(mm,t4.ExtraMonth,T0.DocDate))), @Fecha) >= 91 AND DATEDIFF(DD,cvsv.dbo.ObtenerFechaFinal(dateadd(dd,t4.ExtraDays,dateadd(mm,t4.ExtraMonth,T0.DocDate))), @Fecha) <= 120 THEN (T0.DocTotal - T0.PaidToDate) 
						...
						case 
							...
							when T2.CntctPrsn not like '%DIRECTO%' and DATEDIFF(DD, cvsv.dbo.ObtenerFechaFinal(dateadd(dd,t4.ExtraDays,dateadd(mm,t4.ExtraMonth,T0.DocDate))),@Fecha) >= 121 THEN (T0.DocTotal - T0.PaidToDate) 
 ...
 INNER JOIN cvgt.dbo.OCRD T2 ON T0.CardCode=T2.CardCode
 INNER JOIN cvgt.dbo.OCRG T3 ON T3.GroupCode=T2.GroupCode
 inner join cvgt.dbo.OCTG AS t4 ON T2.GroupNum = t4.GroupNum 
