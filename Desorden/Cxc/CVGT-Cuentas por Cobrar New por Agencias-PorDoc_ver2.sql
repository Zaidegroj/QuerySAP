...
SELECT     T0.CardCode	AS Cliente, T0.CardName AS Nombre, T2.CntctPrsn AS agencia, T1.SeriesName AS Serie, T0.DocNum AS Documento,t4.ExtraMonth,t4.ExtraDays, 
						T0.TaxDate AS Fecha, 
						isnull(T0.u_VttoAgencia,then T0.DocDueDate) end AS Vence,
						...
						case 
							...
							when t0.u_VttoAgencia is not null and DATEDIFF(DD, dateadd(mm,t4.ExtraMonth,T0.u_VttoAgencia)+dateadd(dd,t4.ExtraDays,t0.u_VttoAgencia),@Fecha) >= 121 THEN (T0.DocTotal - T0.PaidToDate) END AS Mas,
						 T3.GroupName AS GRUPO
FROM         cvgt.dbo.OINV AS T0 INNER JOIN
             cvgt.dbo.NNM1 AS T1 ON T0.Series = T1.Series INNER JOIN
             cvgt.dbo.OCRD AS T2 ON T0.CardCode = T2.CardCode INNER JOIN
             cvgt.dbo.OCTG AS t4 ON T0.GroupNum = t4.GroupNum INNER JOIN
             cvgt.dbo.OCRG AS T3 ON T3.GroupCode = T2.GroupCode
...
SELECT     T0.CardCode AS Cliente, T0.CardName AS Nombre, T2.CntctPrsn AS agencia, T1.SeriesName AS Serie, T0.DocNum AS Documento, t4.ExtraMonth,t4.ExtraDays, 
           ...
           (T0.DocTotal - T0.PaidToDate) * - 1 AS Saldo,
			...
			T3.GroupName AS GRUPO
FROM         cvgt.dbo.ORIN AS T0 INNER JOIN
             cvgt.dbo.NNM1 AS T1 ON T0.Series = T1.Series INNER JOIN
             cvgt.dbo.OCRD AS T2 ON T0.CardCode = T2.CardCode INNER JOIN
             cvgt.dbo.OCTG AS t4 ON T0.GroupNum = t4.GroupNum INNER JOIN
             cvgt.dbo.OCRG AS T3 ON T3.GroupCode = T2.GroupCode
...
---select * from ocrd
---SELECT * FROM octg
---select * from oinv where docnum = '101142'