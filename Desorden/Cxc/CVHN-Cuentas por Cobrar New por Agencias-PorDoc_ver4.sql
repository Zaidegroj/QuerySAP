...
FROM         cvhn.dbo.OINV AS T0 INNER JOIN
             cvhn.dbo.NNM1 AS T1 ON T0.Series = T1.Series INNER JOIN
             cvhn.dbo.OCRD AS T2 ON T0.CardCode = T2.CardCode INNER JOIN
             cvhn.dbo.OCTG AS t4 ON T2.GroupNum = t4.GroupNum INNER JOIN
             cvhn.dbo.OCRG AS T3 ON T3.GroupCode = T2.GroupCode
