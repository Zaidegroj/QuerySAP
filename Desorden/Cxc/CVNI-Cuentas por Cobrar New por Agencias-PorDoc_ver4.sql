...
if (@iInDesign)=1
	...
	begin
		/* SELECT FROM cvni.DBO.OINV T1 */
		SET @Fecha = /* T1.DocDate */ '[%0]'
		/* SELECT FROM cvni.DBO.OCRG T2 */
		SET @GrupoIni = /* T2.GroupName */ '[%1]'
		/* SELECT FROM cvni.DBO.OCRG T2 */
		SET @GrupoFin = /* T2.GroupName */ '[%2]'
		/* SELECT FROM cvni.DBO.ORTT T0 */
		SET @tc    = /* T0.Rate    */ '[%3]'
	end
...
FROM         cvni.dbo.OINV AS T0 INNER JOIN
             cvni.dbo.NNM1 AS T1 ON T0.Series = T1.Series INNER JOIN
             cvni.dbo.OCRD AS T2 ON T0.CardCode = T2.CardCode INNER JOIN
             cvni.dbo.OCTG AS t4 ON T2.GroupNum = t4.GroupNum INNER JOIN
             cvni.dbo.OCRG AS T3 ON T3.GroupCode = T2.GroupCode
