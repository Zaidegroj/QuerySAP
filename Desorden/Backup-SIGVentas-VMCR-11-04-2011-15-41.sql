...
--set @dFechaIni = '05/01/2010 00:00:00'
...
SELECT '100'                                             AS Grupo    ,
 ...
 WHERE T1.DocDate   >= @dFechaIni
   AND T1.DocDate   <= @dFechaFin
   AND T1.DocType    = 'S'
   AND T5.GroupCode <> '104' AND T5.GroupCode <> '107' AND T5.GroupCode <> '108'
...
INSERT INTO #Tmp_Conso
/*LO QUE SE MUESTRA EN SERVICIO ES EXACTAMENT LO DE THETRICAL Y CLIENTES CINEVISION NADA MAS Y ALGUN DOC Q ES DE CLIENTE Q NO ES ND*/
SELECT Grupo,Nombre,round(SUM(Bruto)/@nTc,2),round(SUM(Descuento)/@nTc,2),round(SUM(Neto)/@nTc,2),
		SUM(Cantidad),round(SUM(PrecioPromedio)/@nTc,2) FROM(
SELECT ' '                                             AS Grupo    ,
