ALTER procedure [dbo].[EstadoResultadoxMeses] (@DBPais as varchar(4),
...
if ((select count(codigo)
	...
	begin	
		...
		insert into #ESMeses 
		select ' ' as codigo,'  TOTAL COSTO DE VENTA POR PRODUCTOS ' as nombre,isnull(sum(enero),0)*-1,
...
if ((select count(codigo)
	...
	begin
		insert into #ESMeses
			...
			group by codigo,nombre
		insert into #ESMeses
				select '' as codigo,'  TOTAL GASTOS DE NO OPERACION ' as nombre,isnull(sum(enero),0),
					isnull(sum(febrero),0),isnull(sum(marzo),0),isnull(sum(abril),0),
						isnull(sum(mayo),0),isnull(sum(junio),0),isnull(sum(julio),0),
						isnull(sum(agosto),0),isnull(sum(septiembre),0),isnull(sum(octubre),0),
						isnull(sum(noviembre),0),isnull(sum(diciembre),0),0,'T6201' as TipoCol
				from  #ESMeses
				where TipoCol = 'G6201'
	end
